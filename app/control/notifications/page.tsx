...
import type { NotificationData } from '@/components/control/notifications/notification-item';
import { cn } from '@/lib/utils';
import { Bell, Clock, MessageSquare, ExternalLink } from 'lucide-react';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
function createNotificationId(index: number): string {
  return `notification-${Date.now()}-${index}`;
}

function parseNotificationDate(value?: string | Date): Date {
...
function normalizeNotification(
  ...
  return {
    id: item.id || item._id || createNotificationId(index),
    type: item.type || ('info' as NotificationData['type']),
    title: item.title || 'Thông báo',
    message: item.message || '',
    timestamp: parseNotificationDate(item.timestamp || item.createdAt),
...
export default function NotificationsPage() {
  ...
  const groupList = Object.entries(groupedNotifications)
      .map(([title, items]) => {
  ...
  if (loading) {
    return (
        <div className="flex-1 bg-white dark:bg-[#0a0a0c] h-full flex items-center justify-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#36caf1]" />
        </div>
    );
  }
  ...
  return (
      <div className="flex-1 bg-white dark:bg-[#0a0a0c] h-full flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-4 h-[60vh]">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-slate-500 max-w-xs">
                      Hệ thống sẽ gửi thông báo cho bạn khi có cập
                      nhật mới.
                    </p>
                  </div>
                </div>
        ...
        >
          <SheetContent
              side="right"
              className="sm:max-w-md border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-0 overflow-hidden flex flex-col"
          >
            {selectedGroup && (
                <>
                  <SheetHeader className="p-6 border-b border-slate-100 dark:border-slate-900 sticky top-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md z-10">
                    ...
                    <div className="mt-2">
                                    <span className="text-[10px] font-bold px-2 py-0.5 bg-[#36caf1]/10 text-[#36caf1] rounded-full">
                                        {selectedGroupItems.length} mục liên
                                        quan
                                    </span>
                    </div>
                  </SheetHeader>
                  ...
                  <div className="flex-1 p-4 space-y-4 overflow-y-auto no-scrollbar bg-slate-50/50 dark:bg-slate-900/10">
                    {selectedGroupItems.map((notification) => (
                        <div
                            key={notification.id}
                            onClick={() =>
